...
const pt_images = {
    ...
    names: [
        "Mr. Machido Dakama", "Mr. Samuel", "Mr. Ali", "name", "name"
    ]
}
...
let slide_testimony = () => {
    ...
    pupil_image.src = `../frontend/static/images/testifier_images/${p_images.images[0]}.jpg`;
    pupil_statement.innerHTML = p_images.statements[p_index].length > 225 ? p_images.statements[p_index].substring(0, 221) + '...' : p_images.statements[p_index];
    pupil_name.innerHTML = p_images.names[p_index];
    if (pt_index >= pt_threshold) {
        pt_index = 0;
    }
    parent_image.src = `../frontend/static/images/testifier_images/${pt_images.images[0]}.jpg`;
    parent_statement.innerHTML = pt_images.statements[pt_index].length > 225 ? pt_images.statements[pt_index].substring(0, 221) + '...' : pt_images.statements[pt_index];
